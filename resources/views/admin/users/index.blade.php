                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-5 py-3">
                            <div class="flex items-center justify-end gap-1">
                                ...
                                <button type="button" 
                                        onclick="toggleUserStatus({{ $user->id }}, '{{ $user->name }}', {{ $user->status ? 'true' : 'false' }})"
                                        title="{{ $user->status ? 'Desactivar' : 'Activar' }}"
                                        class="p-1.5 text-gray-400 hover:text-{{ $user->status ? 'orange' : 'green' }}-600 hover:bg-{{ $user->status ? 'orange' : 'green' }}-50 rounded-lg transition-colors">
                                    @if($user->status)
                                    {{-- Ícono Desactivar --}}
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <form method="POST" action="{{ route('admin.users.destroy', $user) }}"
                                      data-confirm="¿Eliminar permanentemente a {{ $user->name }} de la base de datos? Esta acción NO se puede deshacer."
...
function toggleStatus(btn, userId) {
    ...
    .then(data => {
        ...
        btn.className = 'inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium transition-colors cursor-pointer ';
        btn.className += data.status ? 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200' : 'bg-gray-100 text-gray-500 hover:bg-gray-200';
        
        // Actualizar también el botón de acción
        location.reload();
    });
}
...
function toggleUserStatus(userId, userName, currentStatus) {
    ...
    Swal.fire({
        ...
        html: `¿Deseas ${action} a <strong>${userName}</strong>?`,
        icon: 'question',
