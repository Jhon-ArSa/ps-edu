...
@section('breadcrumb')
    ...
    <svg class="w-3.5 h-3.5 mx-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
    <span class="text-gray-700 font-medium">Nueva mención</span>
@endsection

@section('content')
<div class="max-w-3xl mx-auto space-y-6" x-data="mentionForm()">

    {{-- Info banner --}}
    <div class="bg-rose-50 border border-rose-200/60 rounded-xl p-4 flex items-start gap-3 animate-fade-in-up">
        ...
        <div>
            <p class="text-sm font-semibold text-rose-800">Mención para: {{ $program->name }}</p>
            <p class="text-xs text-rose-600 mt-0.5">Define una mención (especialización) y su plan de estudios organizado por semestres.</p>
        </div>
    </div>

    <form method="POST" action="{{ route('admin.programs.mentions.store', $program) }}" class="space-y-5">
        @csrf
        ...
        <div class="card animate-fade-in-up delay-1">
            ...
            <div class="px-6 py-5 space-y-4">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required placeholder="Ej: Gestión Educativa"
                           class="form-input w-full">
                    @error('name') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="form-label">Descripción</label>
                    <textarea name="description" rows="3" placeholder="Descripción breve de la mención..." class="form-input w-full resize-none">{{ old('description') }}</textarea>
        ...
        <div class="flex items-center justify-between animate-fade-in-up delay-3">
            ...
            <button type="submit"
                    class="inline-flex items-center gap-2 bg-gradient-to-r from-rose-600 to-rose-700 hover:from-rose-700 hover:to-rose-800 text-white text-sm font-bold px-6 py-3 rounded-xl shadow-lg shadow-rose-600/25 transition-all hover:shadow-xl hover:shadow-rose-600/30 hover:-translate-y-0.5 active:translate-y-0">
