...
@section('content')
<div x-data="courseForm()" class="max-w-4xl mx-auto space-y-6">

    {{-- Header --}}
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl font-bold text-gray-900">Crear nuevo curso</h1>
            <p class="text-sm text-gray-500 mt-0.5">Complete la información del curso, asigne un docente y matricule alumnos.</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('admin.courses.store') }}" class="space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" required
                               ...
                               placeholder="Ej: Metodología de la Investigación Científica">
                    ...
                    <div>
                        ...
                        <div class="flex gap-3">
                            <label class="flex-1 relative cursor-pointer">
                                <input type="radio" name="status" value="active" {{ old('status', 'active') === 'active' ? 'checked' : '' }} class="peer sr-only">
                            ...
                            <label class="flex-1 relative cursor-pointer">
                                <input type="radio" name="status" value="inactive" {{ old('status') === 'inactive' ? 'checked' : '' }} class="peer sr-only">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea name="description" rows="3"
                                  class="w-full px-3.5 py-2.5 rounded-xl border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-primary-400 focus:border-primary-400 resize-none transition-shadow"
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <select name="semester_id"
                            ...
                            @foreach($semesters as $sem)
                                <option value="{{ $sem->id }}" {{ old('semester_id', request('semester_id')) == $sem->id ? 'selected' : '' }}>
                                    {{ $sem->name }}
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <div class="flex gap-3">
                            <label class="flex-1 relative cursor-pointer">
                                <input type="radio" name="semester" value="I" {{ old('semester') === 'I' ? 'checked' : '' }} class="peer sr-only">
                            ...
                            <label class="flex-1 relative cursor-pointer">
                                <input type="radio" name="semester" value="II" {{ old('semester') === 'II' ? 'checked' : '' }} class="peer sr-only">
                                <div class="flex flex-col items-center gap-1 px-4 py-3 rounded-xl border-2 border-gray-200 text-sm peer-checked:border-primary-500 peer-checked:bg-primary-50 peer-checked:text-primary-700 transition-all">
                            ...
                            <label class="flex-1 relative cursor-pointer">
                                <input type="radio" name="semester" value="" {{ old('semester') === null || old('semester') === '' ? 'checked' : '' }} class="peer sr-only">
        ...
        <div class="card animate-fade-in-up delay-2" style="overflow: visible; z-index: 30; position: relative">
            ...
            <div class="p-6">
                {{-- Teacher search --}}
                <div x-data="{ teacherSearch: '', open: false }" class="relative" @click.away="open = false">
                    ...
                    <div x-show="open" x-transition.opacity class="absolute z-20 mt-1.5 w-full bg-white rounded-xl border border-gray-200 shadow-xl shadow-gray-200/50 max-h-52 overflow-y-auto">
                        ...
                        @endforeach
                        <div x-show="teacherSearch && ![{{ $teachers->map(fn($t) => "'" . strtolower($t->name) . "'")->join(',') }}].some(n => n.includes(teacherSearch.toLowerCase()))"
                ...
                @if(old('teacher_id'))
                    @php $selectedTeacher = $teachers->firstWhere('id', old('teacher_id')); @endphp
                    @if($selectedTeacher)
                    <div class="mt-4 flex items-center gap-3 p-3 rounded-xl bg-amber-50 border border-amber-200">
                        <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-amber-400 to-amber-500 flex items-center justify-center shrink-0">
                            <span class="text-white text-sm font-bold">{{ strtoupper(substr($selectedTeacher->name, 0, 2)) }}</span>
                        </div>
                        <div>
                            <p class="text-sm font-semibold text-amber-900">{{ $selectedTeacher->name }}</p>
                            <p class="text-xs text-amber-600">{{ $selectedTeacher->email }}</p>
                        </div>
                        <span class="ml-auto text-xs font-medium text-amber-700 bg-amber-200 px-2 py-0.5 rounded-full">Seleccionado</span>
                    </div>
                    @endif
                @endif
            </div>
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-400">Seleccione los alumnos a matricular en este curso</p>
                    </div>
                </div>
            ...
            <div class="p-6 space-y-4">
                ...
                <div class="border border-gray-200 rounded-xl overflow-hidden max-h-72 overflow-y-auto">
                    ...
                    <label x-show="matchStudent({{ json_encode(['name' => $student->name, 'email' => $student->email, 'dni' => $student->dni ?? '']) }})"
                        ...
                        @if($student->alumnoProfile && $student->alumnoProfile->program)
                        <span class="hidden sm:inline text-[10px] text-gray-400 bg-gray-50 px-2 py-0.5 rounded-full truncate max-w-[140px]">{{ $student->alumnoProfile->program }}</span>
                        @endif
                    </label>
                    @empty
...
function courseForm() {
    return {
        studentSearch: '',
        selectedStudents: @json(old('students', [])).map(String),
