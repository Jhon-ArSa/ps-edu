    ...
    <div class="space-y-8">
        ...
        <div class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-gray-100 via-gray-50 to-white border-2 border-gray-200 shadow-xl">
            ...
            <div class="relative p-16 text-center">
                ...
                <p class="text-gray-500 font-medium max-w-md mx-auto">Aun no hay comunicados publicados para docentes. Vuelve pronto para ver las novedades.</p>
            </div>
        </div>
        @endforelse
    </div>
...
@if($popupAnnouncements->isNotEmpty())
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const queue = ids.filter(id =>
            !localStorage.getItem(`announcement_${id}_shown`) &&
            !localStorage.getItem(`announcement_${id}_read`)
        );
        let index = 0;
        function showNext() {
            if (index < queue.length) {
                document.dispatchEvent(new CustomEvent(`show-popup-${queue[index++]}`));
            }
        }
