        ...
        <form method="POST" action="{{ route('docente.soporte.send') }}" class="space-y-4">
            ...
            <div>
                ...
                <select name="category" required
                        class="w-full px-3 py-2.5 rounded-lg border {{ $errors->has('category') ? 'border-red-400' : 'border-gray-300' }} text-sm focus:outline-none focus:ring-2 focus:ring-primary-400">
                    <option value="">Selecciona una categoría</option>
                    <option value="tecnico" {{ old('category') === 'tecnico' ? 'selected' : '' }}>Problema técnico</option>
                    <option value="acceso" {{ old('category') === 'acceso' ? 'selected' : '' }}>Acceso / Contraseña</option>
                    <option value="academico" {{ old('category') === 'academico' ? 'selected' : '' }}>Consulta académica</option>
